...
class RunDb:
    ...
    def calc_itp(self, run, count):
        # Tests default to 100% base thruput, but we have several adjustments behind the scenes to get internal thruput.
        base_tp = run["args"]["throughput"]
        itp = base_tp = max(min(itp, 500), 1) # Sanity check
        ...
        # Finally two additive bonuses for LLR and long-running tests. They're additive so that the bonuses are similar
        # for both outlier base_tps and typical base_tps.
        llr = run["args"].get("sprt", {}).get("llr", 0)
        # Don't throw workers at a run that finishes in 2 minutes anyways
        llr = min(max(llr, 0), 2.0)
        a = 3  # max LLR bonus +67%
        bonus = (llr + a) / a
        itp += (bonus - 1) * 100
        # max long test bonus +100%
        if "sprt" in run["args"]:
            r = run["results"]
    ...
    def sync_request_task(self, worker_info):
        ...
        def priority(run):  # lower is better
            return (
                ...
                # This does result in some bias where low itp tests are more likely
                # to get smaller workers and vice versa, however no one has *yet*
                # demonstrated any resulting data quality issues (but perhaps we should
                # investigate that either way -- OTOH this is hardly the only
                # source of bias either).
                run["cores"] > 0,
